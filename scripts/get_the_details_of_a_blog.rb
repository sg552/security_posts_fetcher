...
blogs.each do |blog|
  ...
  if images != ''
    images.to_ary.each do |image|
      ...
      image_name = image_src.sub('https://xzfile.aliyuncs.com/media/upload/picture/', '')
      puts "=== image_src_sub is #{image_name}"
      #image_url_local = "public/blog_images/#{blog.id}/"
      #`mkdir public/blog_images && mkdir "#{image_url_local}"`
      #`wget -c0 - "#{image_src}" > "#{image_url_local}/#{image_name}"`
      `wget -cO - "#{image_src}" > "public/blog_images/#{image_name}"`
      #puts "=== image_url_local #{image_url_local}"
    end
  end
  blog_content = to_get_content.to_s.gsub("https://xzfile.aliyuncs.com/media/upload/picture/", "public/blog_images/")
  puts "=== blog_content #{blog_content}"
